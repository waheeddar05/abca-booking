...
An alert about **someone else's** booking is a staff / role-subscriber one, and `GET /api/notifications` flags it with `isOwnBooking: false`. The view then renders the card in the `operator` role (the `user` role hides the booker's phone and its `tel:` link — the one thing floor staff act on) **and** keeps the message text underneath, because the card is a snapshot of the first row only: the whole-batch time window, the batch total and the cancelled-by / reason lines live in the message.
