...
**Where it is highlighted**: Navbar desktop link + BottomNav tab "Shop" (a "Soon" pill on desktop, an amber dot on the phone tab, while `comingSoon`), the landing page's **GEAR UP.** section (`LandingShopSection` — up to four published products, featured first, via `GET /api/shop/status`, or category teaser tiles when the catalog is empty) and a "Shop" pill in the landing nav, and a dismissible `MarketplacePromoBanner` strip at the top of `/slots`. All of them read one cached client fetch of `/api/shop/status` through `useMarketplaceStatus()` (`src/lib/marketplace-status.tsx`); the admin settings card calls `invalidateMarketplaceStatus()` after a save, which refetches and pushes the new status to every mounted consumer (the Navbar and BottomNav live in the root layout and never remount). The status route is `no-store` — its payload depends on the center cookie, so a browser-cached copy would replay the previous center's store after a switch.

**"Notify me" is a lead list, not an automated message.** The interest rows surface on Admin → Marketplace as a count per product and a list of names and numbers; there is no job that messages them when `comingSoon` flips or stock arrives — the admin follows up over WhatsApp from that list. Build the automation before promising it in copy.
