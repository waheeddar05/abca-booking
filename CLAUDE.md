...
**MODERATOR** is a restricted admin (both a `UserRole` and a `MembershipRole`): same access as ADMIN except it cannot reach the Users, Settings, or My Center surfaces, cannot cancel/refund bookings or reassign booking staff, and cannot mutate packages (create/edit/activate, user-package changes, or assign). The staff-management tabs (Sidearm / Personal Coach / Ground Staff) are fully open to moderators with admin parity — their members API (`requireCenterStaffManagerForCenter` in `src/lib/adminAuth.ts`) admits moderators but scopes them to the COACH / SIDEARM_SPECIALIST / GROUND_STAFF roles. Enforced in the middleware (`/admin` path blocklist), the API guards, and the admin UI. Detect it via `isCenterModerator(user, centerId)` (`src/lib/auth.ts`) on the backend or the `isModerator` flag from `requireCenterAdmin`; on the client use `useAdminRole()` (`src/lib/useAdminRole.ts`).
