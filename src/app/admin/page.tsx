...
interface Stats {
  ...
  /** Hand-entered revenue from Admin → Ledger; part of totalRevenue. */
...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  // Manual revenue with no service behind it (Admin → Ledger, category
  // "Other / Miscellaneous"). Every other ledger category merges into
  // the booking category bars above.
  OTHER: 'Other / Misc',
};
...
// Categories shown on the Booking Distribution table. Match Practice
// categories included so corporate-batch enrollments and
// match-simulation seats show up in the counts.
const DASHBOARD_CATEGORIES = [
  'MACHINE', 'SIDEARM', 'COACHING', 'NET', 'FULL_COURT', 'CORPORATE_BATCH', 'MATCH_SIMULATION',
];

// Buckets on the Revenue by Category chart. Manual revenue from the
// Ledger merges into these same booking-category bars — a hand-recorded
// sidearm session sits on the Sidearm bar next to the booked ones — so
// the only extra bucket is OTHER, for manual income with no service
// behind it. OTHER never appears in the Booking Distribution table
// above, which counts bookings and has no such category.
const REVENUE_CHART_CATEGORIES = [...DASHBOARD_CATEGORIES, 'OTHER'];
...
export default function AdminDashboard() {
  ...
  const revenueByCategoryData = (stats?.revenueBreakdown?.entries || [])
    .filter(entry => entry && REVENUE_CHART_CATEGORIES.includes(entry.key))
    .map(entry => ({
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-2 sm:gap-4">
        ...
        <AdminStatCard
          ...
          value={stats?.manualRevenue ? `₹${stats.manualRevenue.toLocaleString()}` : '₹0'}
          icon={IndianRupee}
          href="/admin/ledger"
          gradient="bg-gradient-to-br from-teal-500/20 to-teal-500/5"
        ...
        <AdminStatCard
          label="Expenses"
          value={stats?.manualExpenses ? `₹${stats.manualExpenses.toLocaleString()}` : '₹0'}
          icon={IndianRupee}
          href="/admin/ledger"
          ...
          <div className="h-60 sm:h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Bar dataKey="revenue" radius={isCompact ? [3, 3, 0, 0] : [4, 4, 0, 0]} barSize={isCompact ? 14 : 36} name="Revenue">
