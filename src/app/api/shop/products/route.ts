...
/**
 * GET /api/shop/products?category=BAT&q=kashmir&limit=24
 *
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!config.enabled) {
      return NextResponse.json({ ...shared, products: [], categories: [] });
    }
    ...
    const limit = Number.isFinite(limitRaw) && limitRaw > 0 ? Math.min(MAX_LIMIT, Math.floor(limitRaw)) : MAX_LIMIT;
