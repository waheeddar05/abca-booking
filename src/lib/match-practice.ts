...
export interface MatchSimulationSession {
  /** Stable id generated by the admin editor. Regular (per-session) seats
   *  are counted by (date, startTime); monthly memberships are counted by
   *  this id + enrollment period (persisted on Booking.matchSimSessionId),
   *  so renaming/re-timing a session never orphans existing rows. */
  id: string;
  label: string;
  days: number[]; // IST 0..6
  startTime: string; // "HH:MM" IST
  ...
  wicketsHeld?: WicketsHeld;
  // ─── Monthly / half-month enrollment (mirrors Corporate Batch) ─────
  /** Whether this session offers a flat Monthly pass (and, when the
   *  half-month split is enabled below, half-month passes). Regular
   *  per-session booking is always available regardless. */
  monthlyEnabled: boolean;
  /** ₹ fee for a full-month pass. Seats are shared with regular attendees. */
  monthlyFee: number;
  /** Optional half-month split for this session — same shape and semantics
   *  as Corporate Batch. Only offered when both monthlyEnabled and
   *  halfMonth.enabled are true. */
  halfMonth: HalfMonthConfig;
}
  ...
  sessions: [
    {
      ...
      wicketsHeld: { ASTRO: 1 },
      // Monthly passes are opt-in per session — a session offers only
      // regular (per-date) booking until the admin turns Monthly on.
...
export function normalizeMatchSimulationSettings(raw: unknown): MatchSimulationSettings {
  ...
  const num = (v: unknown, fb: number) =>
    typeof v === 'number' && Number.isFinite(v) && v >= 0 ? v : fb;
  const dHalf = DEFAULT_MATCH_SIMULATION_SETTINGS.sessions[0].halfMonth;
  ...
  const sessions: MatchSimulationSession[] = rawSessions
    ? rawSessions
        ...
        .map((s, i) => {
          const half = (s as { halfMonth?: Partial<HalfMonthConfig> }).halfMonth;
          return {
            ...
            // Monthly / half-month knobs — absent on sessions saved before
            // this feature, so they fall back to "regular only" defaults.
            monthlyEnabled: typeof s.monthlyEnabled === 'boolean' ? s.monthlyEnabled : false,
            monthlyFee: num(s.monthlyFee, 1500),
            halfMonth: {
              enabled: typeof half?.enabled === 'boolean' ? half.enabled : dHalf.enabled,
              fee: num(half?.fee, dHalf.fee),
              firstHalf: typeof half?.firstHalf === 'boolean' ? half.firstHalf : dHalf.firstHalf,
              secondHalf: typeof half?.secondHalf === 'boolean' ? half.secondHalf : dHalf.secondHalf,
              splitDay: Math.min(27, Math.max(1, num(half?.splitDay, dHalf.splitDay))),
            },
          };
        })
    : d.sessions.map((s) => ({ ...s, halfMonth: { ...s.halfMonth } }));
  return {
    enabled: typeof r.enabled === 'boolean' ? r.enabled : d.enabled,
    sessions,
  };
...
/** Monthly / half-month members enrolled for a match-simulation session
 *  for periods overlapping `period` (a full month counts H1 + H2 +
 *  monthly; a half counts monthly + that half). Scoped to one session via
 *  matchSimSessionId so two sessions never pool their memberships. */
export async function countMatchSimMonthlyMembers(
  client: BookingCountClient,
  centerId: string,
  sessionId: string,
  period: string,
  ...
  return client.booking.count({
    where: {
      ...
      status: { not: 'CANCELLED' },
      matchSimSessionId: sessionId,
      enrollmentPeriod: { in: overlappingPeriods(period) },
    },
  });
}

/** Seats taken for one match-simulation session occurrence =
 *  regular (per-session) rows on that exact (date, startTime) +
 *  monthly / half-month members whose period covers the date.
 *  Legacy rows (no matchSimSessionId) are counted via the regular
 *  (date, startTime) leg, so nothing about existing bookings changes. */
export async function countMatchSimSeats(
  ...
  const [regular, members] = await Promise.all([
    client.booking.count({
      where: {
        centerId,
        category: 'MATCH_SIMULATION',
        ...
        date: dateStringToUTC(dateStr),
        startTime,
        // Regular seats only — monthly members are counted below so they
        // aren't double-counted on their membership's "starts on" date.
...
export interface MatchPracticePlan {
  ...
  /** Purchase mode. CORPORATE_BATCH: MONTHLY / HALF_MONTH / REGULAR.
   *  MATCH_SIMULATION: MONTHLY / HALF_MONTH for enrollments, null for a
   *  regular (per-session) seat. */
  mode: CorporateMode | null;
  /** Set for MONTHLY / HALF_MONTH; null otherwise. */
  enrollmentPeriod: string | null;
  /** Which MATCH_SIMULATION session this seat belongs to (config session
   *  id, persisted on Booking.matchSimSessionId). Null for CORPORATE_BATCH. */
  matchSimSessionId: string | null;
  dateStr: string;
...
export interface MatchPracticeBookingRequest {
  category: MatchPracticeCategory;
  /** Purchase mode. For CORPORATE_BATCH this is the corporate mode; for
   *  MATCH_SIMULATION, MONTHLY / HALF_MONTH signal a session enrollment
   *  (with `matchSimSessionId` + `enrollmentPeriod`), and null/omitted
   *  means a regular per-session booking. */
  corporateMode?: CorporateMode | null;
  enrollmentPeriod?: string | null;
  /** Required for a MATCH_SIMULATION MONTHLY / HALF_MONTH enrollment —
   *  which configured session the pass is for. Ignored otherwise. */
  matchSimSessionId?: string | null;
...
export async function resolveMatchPracticePlans(
  ...
  // ── Monthly / half-month enrollment for a single session ───────────
  // Mirrors the corporate-batch enrollment flow, but the pass is tied to
  // ONE configured session (identified by matchSimSessionId) so its fee,
  // capacity, days and half-month split all come from that session.
  if (body.corporateMode === 'MONTHLY' || body.corporateMode === 'HALF_MONTH') {
    const mode = body.corporateMode;
    const session = enabledSessions.find((s) => s.id === body.matchSimSessionId);
    if (!session || !session.monthlyEnabled) {
      throw new BookingResourceError('This session does not offer monthly passes', 400);
    }
    const period = body.enrollmentPeriod ?? '';
    const parsed = parseEnrollmentPeriod(period);
    if (!parsed || (mode === 'MONTHLY') !== (parsed.half === null)) {
    ...
    if (mode === 'HALF_MONTH') {
      if (!session.halfMonth.enabled) {
        throw new BookingResourceError('Half-month enrollment is not enabled for this session', 400);
      }
      if (parsed.half === 'H1' && !session.halfMonth.firstHalf) {
        throw new BookingResourceError('First-half enrollment is not available', 400);
      }
      if (parsed.half === 'H2' && !session.halfMonth.secondHalf) {
        throw new BookingResourceError('Second-half enrollment is not available', 400);
      }
    }
    const monthKey = period.slice(0, 7);
    if (!listMonthKeys(12, todayIST).includes(monthKey)) {
      throw new BookingResourceError('Enrollment is only open for the current and upcoming months', 400);
    }
    // firstUpcomingSessionInPeriod reads days/startTime/endTime/halfMonth —
    // a match-sim session satisfies that shape directly.
    const first = firstUpcomingSessionInPeriod(session, period, now);
    if (!first) {
      throw new BookingResourceError(
        `No upcoming ${session.label || 'match simulation'} sessions in ${formatPeriodLabel(period)}`,
        400,
      );
    }
    return [{
      ...
      mode,
      enrollmentPeriod: period,
      matchSimSessionId: session.id,
      dateStr: first.date,
      date: dateStringToUTC(first.date),
      startTime: first.startTime,
      endTime: first.endTime,
      fee: mode === 'MONTHLY' ? session.monthlyFee : session.halfMonth.fee,
      capacity: session.capacity,
      label: session.label
        ? `${session.label} · ${formatPeriodLabel(period)}`
        : formatPeriodLabel(period),
      coachName: session.coachName || null,
      splitDay: session.halfMonth.splitDay,
    }];
  }

  // ── Regular (per-session) booking — existing behaviour ─────────────
  if (body.slots.length === 0) {
    throw new BookingResourceError('Select at least one session', 400);
  }
...
export async function assertMatchPracticeSeat(
  ...
  // MATCH_SIMULATION
  if (plan.enrollmentPeriod) {
    // Monthly / half-month enrollment for a session.
    const dup = await tx.booking.findFirst({
      where: {
        centerId,
        userId,
        category: 'MATCH_SIMULATION',
        status: { not: 'CANCELLED' },
        matchSimSessionId: plan.matchSimSessionId,
        enrollmentPeriod: { in: overlappingPeriods(plan.enrollmentPeriod) },
      },
      select: { id: true },
    });
    ...
    const enrolled = plan.matchSimSessionId
      ? await countMatchSimMonthlyMembers(tx, centerId, plan.matchSimSessionId, plan.enrollmentPeriod)
      : 0;
    if (enrolled >= plan.capacity) {
      throw new BookingResourceError(
        `${formatPeriodLabel(plan.enrollmentPeriod)} batch is full (${enrolled} of ${plan.capacity} enrolled)`,
  ...
  // Regular (per-session) seat. Reject a second seat in the same
  // occurrence, or a booking already covered by a monthly membership.
  const dup = await tx.booking.findFirst({
    where: {
      centerId,
      userId,
      ...
      status: { not: 'CANCELLED' },
      OR: [
        { date: plan.date, startTime: plan.startTime, enrollmentPeriod: null },
        ...(plan.matchSimSessionId
          ? [{
              matchSimSessionId: plan.matchSimSessionId,
              enrollmentPeriod: { in: periodsCoveringDate(plan.dateStr, plan.splitDay) },
            }]
  ...
  if (dup) {
    throw new BookingResourceError(
      dup.enrollmentPeriod
        ? 'Your monthly enrollment already covers this session'
        : 'You already have a seat in this session',
      409,
    );
  }
  const seats = await countMatchSimSeats(
    tx, centerId, plan.dateStr, plan.startTime, plan.matchSimSessionId, plan.splitDay,
  );
  if (seats >= plan.capacity) {
    throw new BookingResourceError(
