...
export function PaymentMethodSelector({ selected, onChange, disabled, showWallet }: PaymentMethodSelectorProps) {
    ...
    <div className="flex flex-col gap-3">
      ...
      {showWallet && walletBalance != null && walletBalance > 0 && (
        ...
        >
          <div
            className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
              selected === 'WALLET' ? 'bg-accent/20' : 'bg-white/[0.06]'
            }`}
          >
            <Wallet
              className={`w-4 h-4 ${selected === 'WALLET' ? 'text-accent' : 'text-slate-400'}`}
            />
          </div>
          <div className="text-left">
            <p
              className={`text-sm font-semibold ${
                selected === 'WALLET' ? 'text-accent' : 'text-slate-300'
              }`}
            >
              Pay with Wallet
            </p>
