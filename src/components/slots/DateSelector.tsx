...
export const DateSelector = memo(function DateSelector({ selectedDate, onSelect, daysAhead = 15 }: DateSelectorProps) {
  ...
  return (
    <div className="mb-5">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1.5 -mx-4 px-4 scrollbar-hide">
        {dates.map((date, days) => {
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 w-12 py-1 rounded-lg text-center transition-all cursor-pointer ${
                isSelected
            ...
            >
              <div className={`text-[9px] uppercase font-medium leading-tight ${isSelected ? 'text-primary/70' : 'text-slate-500'}`}>
                {isToday ? 'Today' : format(date, 'EEE')}
              </div>
              <div className="text-sm font-bold leading-tight">{format(date, 'd')}</div>
              <div className={`text-[9px] leading-tight ${isSelected ? 'text-primary/60' : 'text-slate-500'}`}>
                {format(date, 'MMM')}
              </div>
            </button>
          );
